					...
					@if($users->count() > 0)
						@foreach($users as $user)
							<tr>
								...
								<td>
									@if($user->admin)
										<a href="{{ route('user.not.admin', ['id' => $user->id]) }}" class="btn btn-danger btn-sm">Remove from Admin</a>
									@else
								...
								<td>
									@if(Auth::id() != $user->id)
										<a href="{{ route('user.destroy', ['id' => $user->id]) }}" class="btn btn-danger btn-sm">Remove User</a>
									@endif
								</td>
							</tr>
						@endforeach
